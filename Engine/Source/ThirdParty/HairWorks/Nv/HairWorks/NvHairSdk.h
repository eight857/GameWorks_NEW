...
Create the hair asset manually from your asset data
	...
	...
	hairSdk->createHairAsset(desc, &hairAssetId);
...
#define NV_HAIR_VERSION 112				// internal version number to match runtime DLLs. 
#define NV_HAIR_VERSION_STRING "1.1.2"	// version string for .apx file format
...
/*! \brief Identifier for a hair instance
	...
	\see Sdk.createHairInstance() and Sdk.freeHairInstance() */
enum InstanceId
{
...
/*! \brief HairWorks file header
	...
	\note If the version is matched, Sdk.loadHairAsset() fails. */
...
/*!\brief Descriptor to specify hair asset (hair curves, growth mesh, skinning weights, bone info...)
	...
	\note When the data is invalid, NvHair::Sdk.loadHairAsset() rejects the hair asset.
...
public:
	...
	/*! \brief Initialize internal render state variables and shaders. On DX11 this will be a ID3D11Device and ID3D11DeviceContext respectively. 
		...
		\return Successful if NV_SUCCEEDED(Result) is true. */
	virtual Result initRenderResources(const NvCo::ApiDevice& device, const NvCo::ApiContext& context = NvCo::ApiContext::getNull()) = 0;
	...
	/*! \brief Create a hair instance from given asset type
		\param [in] assetId hair asset type to create a hair instance from.  
					The assetId should have been generated from a successful call of NvHair::Sdk.loadHairAsset().
	...
	/** \brief render a hair instance to the current render target.
		...
		\return Successful if NV_SUCCEEDED(Result) is true. */
	virtual Result renderHairs(InstanceId instanceId, const ShaderSettings* shaderSettings = NV_NULL) = 0;		
	...
	/*!\brief get statistics about a hair instance
		...
		\return Successful if NV_SUCCEEDED(Result) is true. */
	virtual Result computeStats(InstanceId instanceId, Stats& statsOut) = 0;
	...
	virtual Void getPinMatrix(InstanceId instanceId, Int pinIndex, gfsdk_float4x4& matrixOut) = 0;
};

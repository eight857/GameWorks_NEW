...
#ifndef NV_HW_SHADER_COMMON_H
#define NV_HW_SHADER_COMMON_H

#include "NvHwShaderCommonTypes.h"
...
float NvHw_GetNormalizedNoise(unsigned int hash, NvHw_ConstantBuffer hairConstantBuffer)
{
	unsigned int id = hash % (NvHw_ConstantBuffer::NOISE_TABLE_SIZE * 4);
...
float NvHw_GetSignedNoise(unsigned int hash, NvHw_ConstantBuffer hairConstantBuffer)
{
	float v = NvHw_GetNormalizedNoise(hash, hairConstantBuffer);
	return 2.0f * (v - 0.5f);
}

//////////////////////////////////////////////////////////////////////////////
// return vector noise [-1..1, -1..1, -1..1] from a unique hash id
float3 NvHw_GetVectorNoise(unsigned int seed, NvHw_ConstantBuffer hairConstantBuffer)
{
	float x = NvHw_GetSignedNoise(seed, hairConstantBuffer);
	float y = NvHw_GetSignedNoise(seed + 1229, hairConstantBuffer);
	float z = NvHw_GetSignedNoise(seed + 2131, hairConstantBuffer);

	return float3(x,y,z);
}

//////////////////////////////////////////////////////////////////////////////
float3 NvHw_ComputeHairShading(
...
float3 NvHw_ComputeHairShading(
	float3						Lcolor,
	float3						Ldir,
	NvHw_ShaderAttributes	attr,
	NvHw_Material			mat,
	float3						hairColor
	)
{
	return NvHw_ComputeHairShading(
		Lcolor, Ldir,
...
//////////////////////////////////////////////////////////////////////////////
float NvHw_ComputeStructuredNoise(
	float						noiseCount,
	float						seed,
...
)
{
	...
	float noise0 = NvHw_GetNormalizedNoise(seedSample0, hairConstantBuffer);
	float noise1 = NvHw_GetNormalizedNoise(seedSample1, hairConstantBuffer);
...
//////////////////////////////////////////////////////////////////////////////
float NvHw_ComputeHairDiffuseShading(
	float3		Ldir, // light direction
	float3		T,
	float3		N,
	float		diffuseScale,
...
// Apply glint
//////////////////////////////////////////////////////////////////////////////
void NvHw_ApplyGlint(const float glintScale, const float glint, const float3 lightColor, inout float3 ambientInOut, inout float specularInOut)
{
...
//////////////////////////////////////////////////////////////////////////////
float3 NvHw_ComputeHairShadingWithGlint(
	...
	)
{
	...
	if (glintScale > 0.0f)
	{
		NvHw_ApplyGlint(glintScale, glint, lightColor, ambient, specularSum);
	}
...
//////////////////////////////////////////////////////////////////////////////
float3 NvHw_ComputeHairShadingWithGlint(float3 lightColor, float3 lightDir, float3 lightAmbient, NvHw_ShaderAttributes attr, NvHw_Material mat, float3 hairColor, float glint)
{
	return NvHw_ComputeHairShadingWithGlint(
...
//////////////////////////////////////////////////////////////////////////////
float NvHw_ComputeHairSpecularShading(
	float		hairID,
...
float NvHw_ComputeHairSpecularShading(
	...
	NvHw_Material			mat)
{
	return  NvHw_ComputeHairSpecularShading(
		attr.hairID,
		Ldir,
...
//////////////////////////////////////////////////////////////////////////////////////////////
float NvHw_GetRootTipRatio(const float s, NvHw_Material mat)
{
	float ratio = s;
...
//////////////////////////////////////////////////////////////////////////////////////////////
float3 NvHw_SampleHairColorTex(
	NvHw_ConstantBuffer	hairConstantBuffer,
	NvHw_Material			mat, 
	...
	float3						texcoords)
{
	...
	float ratio = NvHw_GetRootTipRatio(texcoords.z, mat);
...
// Returns hair color from textures for this hair fragment.
//////////////////////////////////////////////////////////////////////////////////////////////
float3 NvHw_SampleHairColorStrandTex(
	NvHw_ConstantBuffer	hairConstantBuffer,
	NvHw_Material			mat, 
	SamplerState				texSampler, 
	Texture2D					rootColorTex, 
...
//////////////////////////////////////////////////////////////////////////////////////////////
float3 NvHw_SampleHairColor(NvHw_Material mat, float4 texcoords)
{
	float3 rootColor = mat.rootColor.rgb;
	float3 tipColor = mat.tipColor.rgb;
...
/////////////////////////////////////////////////////////////////////////////////////////////
float NvHw_SoftDepthCmpGreater(float sampledDepth, float calcDepth)
{
	return max(0.0, sampledDepth - calcDepth);
}

float NvHw_SoftDepthCmpLess(float sampledDepth, float calcDepth)
{
	...
	int shadowUseLeftHanded)
{
	...
	for (int dx = - 1; dx <= 1; dx ++) {
		for (int dy = -1; dy <= 1; dy ++) {
			...
			float shadowDepth = NvHw_SoftDepthCmpLess(sampleDepth, calcDepth);

			if (shadowUseLeftHanded == 0)
				shadowDepth = NvHw_SoftDepthCmpGreater(sampleDepth, calcDepth);
...
/////////////////////////////////////////////////////////////////////////////////////////////
float NvHw_ShadowPenetrationDepth(float sampledDepth, float calcDepth, float gain = -1.0f)
{
...
// Use gain = -1.0f for L.H.S light camera (depth is greater for far object)
/////////////////////////////////////////////////////////////////////////////////////////////
float NvHw_ShadowFilterDepth(Texture2D shadowTexture, SamplerState texSampler, float2 texcoord, float depth, float gain = -1.0f )
{
	...
	for (int dx = - 1; dx <= 1; dx += 2) {
		for (int dy = -1; dy <= 1; dy += 2) {
			...
			filteredDepth += NvHw_ShadowPenetrationDepth(S.x, depth, gain);
			filteredDepth += NvHw_ShadowPenetrationDepth(S.y, depth, gain);
			filteredDepth += NvHw_ShadowPenetrationDepth(S.z, depth, gain);
			filteredDepth += NvHw_ShadowPenetrationDepth(S.w, depth, gain);
...
/////////////////////////////////////////////////////////////////////////////////////////////
float NvHw_ShadowLitFactor(NvHw_ConstantBuffer constBuf, NvHw_Material mat, float filteredDepth)
{
...
//////////////////////////////////////////////////////////////////////////////////////////////
bool NvHw_VisualizeColor(
	NvHw_ConstantBuffer	hairConstantBuffer,
	NvHw_Material			hairMaterial,
	NvHw_ShaderAttributes attr,
...
//////////////////////////////////////////////////////////////////////////////////////////////
float4 NvHw_ScreenToClip(float4 input, NvHw_ConstantBuffer hairConstantBuffer)
{
	float4 sp;
...
//////////////////////////////////////////////////////////////////////////////////////////////
float4 NvHw_ScreenToView(float4 pixelPosition, NvHw_ConstantBuffer hairConstantBuffer)
{
	float4 ndc = NvHw_ScreenToClip(pixelPosition, hairConstantBuffer);
...
//////////////////////////////////////////////////////////////////////////////////////////////
float3 NvHw_ScreenToWorld(float4 pixelPosition, NvHw_ConstantBuffer	hairConstantBuffer)
{
...
//////////////////////////////////////////////////////////////////////////////////////////////
float4 NvHw_WorldToScreen(float3 worldPos, NvHw_ConstantBuffer	hairConstantBuffer)
{
...
//////////////////////////////////////////////////////////////////////////////////////////////
float4 NvHw_WorldToScreenPrev(float3 worldPos, NvHw_ConstantBuffer hairConstantBuffer)
{
...
/////////////////////////////////////////////////////////////////////////////////////////////
float NvHw_PackFloat2(float2 v)
{
...
float2 NvHw_UnpackFloat2(float packedVal)
{
...
float NvHw_PackSignedFloat(float x)
{
	return 0.5f + 0.5f * clamp(x, -1.0, 1.0);
}

float NvHw_PackSignedFloat2(float2 v)
{
	float sx = NvHw_PackSignedFloat(v.x);
	float sy = NvHw_PackSignedFloat(v.y);

	return NvHw_PackFloat2(float2(sx,sy));
}

float NvHw_UnpackSignedFloat(float x)
{
	return clamp(2.0f * (x - 0.5f), -1.0f, 1.0f);
}

float2 NvHw_unpackSignedFloat2(float x)
{
	float2 unpacked = NvHw_UnpackFloat2(x);
	float sx = NvHw_UnpackSignedFloat(unpacked.x);
	float sy = NvHw_UnpackSignedFloat(unpacked.y);
	return float2(sx, sy);
}
...
//////////////////////////////////////////////////////////////////////////////
// DO NOT USE.
NvHw_ShaderAttributes NvHw_GetShaderAttributesInternal(NvHw_PixelShaderInput input, NvHw_ConstantBuffer	hairConstantBuffer)
{
	NvHw_ShaderAttributes attr;

	attr.P		= NvHw_ScreenToWorld(input.position, hairConstantBuffer);

	attr.texcoords.xy = NvHw_UnpackFloat2(input.compTexcoord.x);
...
//////////////////////////////////////////////////////////////////////////////
// DO NOT USE.
#define NV_HW_INDICES_BUFFER_TYPE	Buffer<float3>
#define NV_HW_TANGENT_BUFFER_TYPE	Buffer<float4>
#define NV_HW_NORMAL_BUFFER_TYPE	Buffer<float4>
#define NV_HW_POSITION_BUFFER_TYPE	Buffer<float4>

#define NV_HW_DECLARE_INTERPOLATED_VAR(BUFFER, VAR, INDEX) \
	float3 VAR; \
	{ \
...
#if defined(NV_HW_DECLARE_VELOCITY_ATTR)
#define NV_HW_DECLARE_INTERPOLATED_VELOCITY \
	NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceMasterPositions, P0, vertexIndices0); \
	NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceMasterPositions, P1, vertexIndices1); \
	float3 wP = lerp(P0, P1, hairFrac); \
	\
	NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceMasterPrevPositions, PP0, vertexIndices0); \
	NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceMasterPrevPositions, PP1, vertexIndices1); \
	float3 wPP = lerp(PP0, PP1, hairFrac); \
	\
	float3 sP = NvHw_WorldToScreen(wP, hairConstantBuffer).xyz;\
	float3 sPP = NvHw_WorldToScreenPrev(wPP, hairConstantBuffer).xyz;\
	attr.wVel = sP - sPP;
#else
#define	NV_HW_DECLARE_INTERPOLATED_VELOCITY
#endif
/*
	...
	NvHw_ShaderAttributes NvHw_getShaderAttributes(NvHw_PixelShaderInput input, NvHw_ConstantBuffer	hairConstantBuffer);

	, where input is the pixel shader input and hairConstantBuffer is constant buffer defined for HairWorks.
	The output (NvHw_ShaderAttributes) contains all the attributes needed for hair shading such as
		world position (P), 
		tangent (T), 
		surface normal (N), 
		view vector (V), 
		texture coordinates (texcoords)
		NvHw_getShaderAttributesInternal(INPUT, CBUFFER, NV_HW_RESOUCES_VAR)

		float		hairFrac	= hairCoord - float(vertexID0); \

	*/
#define NV_HW_DECLARE_DEFAULT_SHADER_ATTRIBUTE_FUNC \
	NvHw_ShaderAttributes NvHw_GetShaderAttributes(const NvHw_PixelShaderInput input, const NvHw_ConstantBuffer hairConstantBuffer) \
	{ \
		NvHw_ShaderAttributes attr = NvHw_GetShaderAttributesInternal(input, hairConstantBuffer); \
		\
		...
		\
		int3 hairIndices = floor(NvHw_resourceFaceHairIndices.Load(input.primitiveId)); \
		...
		\
		float2	coords = NvHw_UnpackFloat2(input.coords); \
		\
		NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceTangents, T0, vertexIndices0); \
		NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceTangents, T1, vertexIndices1); \
		attr.T = normalize(lerp(T0, T1, hairFrac)); \
		\
		NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceNormals, N0, vertexIndices0); \
		NV_HW_DECLARE_INTERPOLATED_VAR(NvHw_resourceNormals, N1, vertexIndices1); \
		attr.N = normalize(lerp(N0, N1, hairFrac)); \
		\
		NV_HW_DECLARE_INTERPOLATED_VELOCITY;\
		\
		return attr; \
	} 
...
#if defined(NV_HW_DECLARE_VELOCITY_ATTR) // use pixel velocity attributes

#define NV_HW_DECLARE_SHADER_RESOURCES(SLOT0, SLOT1, SLOT2, SLOT3, SLOT4) \
	NV_HW_INDICES_BUFFER_TYPE	NvHw_resourceFaceHairIndices	: register(SLOT0); \
	NV_HW_TANGENT_BUFFER_TYPE	NvHw_resourceTangents			: register(SLOT1); \
	NV_HW_NORMAL_BUFFER_TYPE	NvHw_resourceNormals			: register(SLOT2); \
	NV_HW_POSITION_BUFFER_TYPE	NvHw_resourceMasterPositions	: register(SLOT3); \
	NV_HW_POSITION_BUFFER_TYPE	NvHw_resourceMasterPrevPositions: register(SLOT4); \
	NV_HW_DECLARE_DEFAULT_SHADER_ATTRIBUTE_FUNC;

#else // don't use pixel velocity attributes

#define NV_HW_DECLARE_SHADER_RESOURCES(SLOT0, SLOT1, SLOT2) \
	NV_HW_INDICES_BUFFER_TYPE	NvHw_resourceFaceHairIndices: register(SLOT0); \
	NV_HW_TANGENT_BUFFER_TYPE	NvHw_resourceTangents		: register(SLOT1); \
	NV_HW_NORMAL_BUFFER_TYPE	NvHw_resourceNormals		: register(SLOT2); \
	NV_HW_DECLARE_DEFAULT_SHADER_ATTRIBUTE_FUNC;

...
inline HairSdk* loadHairSdk(const Char* dllPath, UInt32 version = NV_HW_VERSION, MemoryAllocator* allocator = NV_NULL, Logger* logger = NV_NULL, Int debugMode = 0)
{
	typedef HairSdk* (__cdecl * Func)(UInt32, MemoryAllocator*, Logger*, Int);
	...
	if (hairDllModule)
	{
		...
		if (!createProc)
		{
			...
			if (logger)
			{
				logger->log(LogSeverity::NON_FATAL_ERROR, "HairWorks dll file not found", NV_FUNCTION_NAME, __FILE__, __LINE__);
			}
		}
		else
		{

...
public:
	...
	Array& operator=(const ThisType& rhs);
		...
		/// Indexing operator
	NV_FORCE_INLINE const T& operator[] (IndexT i) const { NV_ASSERT(i >= 0 && i < m_size); return m_data[i]; }
		/// Const indexing operator
	NV_FORCE_INLINE T& operator[] (IndexT i) { NV_ASSERT(i >= 0 && i < m_size); return m_data[i]; }
	...
	NV_FORCE_INLINE const T& front() const { NV_ASSERT(m_size > 0); return m_data[0]; }
		/// Returns a reference to the first element of the array. Undefined if the array is empty.
	NV_FORCE_INLINE T& front() { NV_ASSERT(m_size > 0); return m_data[0]; }

		/// Returns a const reference to the last element of the array. Undefined if the array is empty
	NV_FORCE_INLINE const T& back() const  { NV_ASSERT(m_size); return m_data[m_size - 1]; }
		/// Returns a reference to the last element of the array. Undefined if the array is empty
	NV_FORCE_INLINE T& back() { NV_ASSERT(m_size); return m_data[m_size - 1]; }
...
Array<T>::Array(AliasUserMemory, Void* data, IndexT size, IndexT capacity) :
	...
	m_capacity(capacity)
{
	NV_ASSERT((SizeT(data) & (NV_ALIGN_OF(T) - 1)) == 0);
}
// ---------------------------------------------------------------------------
template<typename T>
Array<T>::~Array()
...
void Array<T>::set(const T* rhsData, IndexT rhsSize)
{
	...
	if (m_size < rhsSize)
	{
		ArrayUtil::assign(m_data, m_data + m_size, rhs.m_data);
...
{
	NV_ASSERT(start < m_size);
	NV_ASSERT((start + count) <= m_size);

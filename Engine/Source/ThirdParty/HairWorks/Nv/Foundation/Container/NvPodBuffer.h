...
#ifndef NV_POD_BUFFER_H
#define NV_POD_BUFFER_H
...
class PodBuffer
{
	...
	NV_FORCE_INLINE const T* end() const { return m_data + m_size; }
		...
		/// [] 
	T& operator[](IndexT i) { NV_ASSERT(i >= 0 && i < m_size); return m_data[i]; }
		/// [] const
	const T& operator[](IndexT i) const { NV_ASSERT(i >= 0 && i < m_size); return m_data[i]; }

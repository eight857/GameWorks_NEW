...
namespace NVRHI
{
	...
	public:
		...
		virtual void* handoffTexture(TextureHandle t);
		virtual TextureHandle getHandleForTexture(void* resource, Format::Enum formatOverride = Format::UNKNOWN);
		...
		void releaseUnmanagedTextures();
		FRHIShader* getRHIShader(ShaderHandle shader);
	...
	private:
		...
		std::map<ID3D12Resource*, FTexture*> m_UnmanagedTextures;

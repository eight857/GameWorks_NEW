...
void FSceneRenderTargets::AllocateCommonDepthTargets(FRHICommandList& RHICmdList)
{
	...
	if (!SceneDepthZ)
	{
		...
		// NvFlow: UE4 bug found, changed TargetableTexture -> ShaderResourceTexture to fix D3D12 crash
		SceneStencilSRV = RHICreateShaderResourceView((FTexture2DRHIRef&)SceneDepthZ->GetRenderTargetItem().ShaderResourceTexture, 0, 1, PF_X24_G8);
	}
	...
	ESamplerFilter ColorFilter ) const
{
	...
	if (TextureMode == ESceneRenderTargetsMode::SetTextures)
	{
		...
		if (SceneContext.IsSeparateTranslucencyPass() && SceneContext.IsSeparateTranslucencyDepthValid())
		{
			...
			if (OutScale < 1.0f)
			{
				SceneDepthTexture = SceneContext.GetSeparateTranslucencyDepthSurface();
			}
		}

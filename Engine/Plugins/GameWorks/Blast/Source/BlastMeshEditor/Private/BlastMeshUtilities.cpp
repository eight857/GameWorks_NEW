	...
	for (uint32_t i = 0; i < FacesCount; i++)
	{
		...
		for (uint32_t j = 0; j < i; j++)
		{
			...
			if (Matches.Num() == 2 || Matches.Num() == 1) // Adjacent faces
			{
				...
				for (int32_t k = 0; k < Matches.Num(); k++)
				{
...
//	Set thresholds to -1 to turn off vertex merging
	BuildOptions.OverlappingThresholds.ThresholdPosition =
	BuildOptions.OverlappingThresholds.ThresholdTangentNormal =
	BuildOptions.OverlappingThresholds.ThresholdUV =
		-1.0f;
...
void CreateSkeletalMeshFromAuthoring(TSharedPtr<FFractureSession> FractureSession, bool isFinal, UMaterialInterface* InteriorMaterial)
{
	...
	IMeshUtilities::MeshBuildOptions BuildOptions;
	//	Set thresholds to -1 to turn off vertex merging if not final
	if (!isFinal)
	{
		BuildOptions.OverlappingThresholds.ThresholdPosition =
		BuildOptions.OverlappingThresholds.ThresholdTangentNormal =

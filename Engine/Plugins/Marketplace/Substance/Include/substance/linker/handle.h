/** @file handle.h
	...
	Outputs management:
	  ...
	  - substanceLinkerHandleSetOutputFormat (change output format)

	Advanced I/O manipulation:
	  - substanceLinkerConnectOutputToInput (connect an image input to an output)
...
/** @brief Change output format and mipmap pyramid depth
	@param substanceLinkerHandle Pointer to the Substance handle.
	...
	@pre The handle must be previously initialized and assembly that contains
		the output referenced by this function must be already pushed.
	@return Returns 0 if succeeded, an error code otherwise. */

...
void USubstanceImageInput::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsSaving())
	{
		ImageRGB.StoreCompressedOnDisk(0 == CompressionLevelRGB ? COMPRESS_ZLIB : COMPRESS_None);
		ImageA.StoreCompressedOnDisk(0 == CompressionLevelAlpha ? COMPRESS_ZLIB : COMPRESS_None);
	}

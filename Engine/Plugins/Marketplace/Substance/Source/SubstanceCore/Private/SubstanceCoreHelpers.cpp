...
void UpdateSubstanceOutput(USubstanceTexture2D* Texture, const SubstanceTexture& ResultText)
{
	...
	if (Texture->Mips.Num() != ResultText.mipmapCount || ResultText.level0Width != Texture->SizeX || ResultText.level0Height != Texture->SizeY)
	{
		Texture->Mips.Empty();
...
void StartPIE()
{
	...
	for (const auto& OutputItr : Substance::Helpers::GetRenderCallbacks()->getComputedOutputs(false))
	{
		...
		if (Result)
		{
			...
			UpdateTexture(Result->getTexture(), OutputItr, false);
			bUpdatedOutput = true;
		}
	}
#endif //WITH_EDITOR
...
void Tick()
{
	...
	for (const auto& OutputItr : Outputs)
	{
		...
		if (Result)
		{
			UpdateTexture(Result->getTexture(), OutputItr);
			bUpdatedOutput = true;
		}
	}
...
{
	if (Input.mDesc.mType != Substance_IType_String)
	{

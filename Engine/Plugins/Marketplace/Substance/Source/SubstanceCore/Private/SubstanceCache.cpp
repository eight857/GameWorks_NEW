...
bool SubstanceCache::CanReadFromCache(std::weak_ptr<SubstanceAir::GraphInstance> graph)
{
#if PLATFORM_PS4
	...
	//make sure all enabled outputs have a cached file
	{
		...
		while (iter != CurrentInstance->getOutputs().end())
		{
			if ((*iter)->mEnabled && (*iter)->mUserData != 0)
			{
				...
				FString path = GetPathForGuid(CurrentGuid);
...
bool SubstanceCache::ReadFromCache(std::weak_ptr<SubstanceAir::GraphInstance> graph)
{
#if PLATFORM_PS4
	// disable caching for PS4
	return false;
...
void SubstanceCache::CacheOutput(SubstanceAir::OutputInstance* output, const SubstanceTexture& result)
{
#if PLATFORM_PS4
	return;
#endif //!PLATFORM_PS4
	...
	FString filename = GetPathForGuid(CurrentGuid);
